...
class SAM2ImagePredictor:
    ...
    def predict(
        ...
        # # convert gra to tensor matching batch size
        # if isinstance(gra, float):
        #     # Match the batch size from point_coords if available
        #     batch_size = 1
        #     if point_coords is not None:
        #         batch_size = point_coords.shape[0]
        #     gra = torch.full((batch_size,), gra, dtype=torch.float, device=self.device)
        masks, iou_predictions, low_res_masks = self._predict(
            unnorm_coords,
            labels,
            unnorm_box,
    ...
    def _predict(
        ...
        img_idx: int = -1,
        gra: float = 1.0,
        granularity: Optional[torch.Tensor] = None
        ...
        if isinstance(gra, float):
            ...
            # print(f"DEBUG: gra.shape={gra.shape}")
