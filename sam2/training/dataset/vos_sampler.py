...
class SampledFramesAndObjects:
    frames: List[int]
    object_ids: List[int]
    sampled_granularities: Optional[Dict[int, Dict[int, float]]] = None  # {frame_idx: {object_id: sampled_value}}
...
class RandomUniformSampler(VOSSampler):
    ...
    def sample(self, video, segment_loader, epoch=None):
        ...
        object_ids = None
        max_select = min(len(visible_object_ids), self.max_num_objects)
        if not object_ids:
            object_ids = random.sample(
                visible_object_ids,
                max_select,
            )
        sampled_granularities = None

        return SampledFramesAndObjects(frames=frames, object_ids=object_ids, sampled_granularities=sampled_granularities)
...
class EvalSampler(VOSSampler):
    ...
    def sample(self, video, segment_loader, epoch=None):
        ...
        return SampledFramesAndObjects(frames=frames, object_ids=list(object_ids), sampled_granularities=None)
