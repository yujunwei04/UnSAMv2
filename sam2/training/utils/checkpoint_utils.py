...
def split_lora_state_dict(
    state_dict: Dict[str, torch.Tensor]
) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
    """Split a state dict into base parameters and LoRA-specific parameters.

    When loading checkpoints saved with LoRA enabled, linear layers are stored under
    ``*.original_layer.*`` while LoRA matrices use the ``lora_`` prefix. This helper
    strips the ``.original_layer`` indirection so the base parameters can be loaded
    into a model that has not yet wrapped its layers with LoRA modules.
    """
    ...
    for key, value in state_dict.items():
        ...
        if ".original_layer." in key:
            base_key = key.replace(".original_layer.", ".")
        else:
            base_key = key
        base_state[base_key] = value

    return base_state, lora_state


def apply_lora_state_dict(
    model: nn.Module, lora_state: Dict[str, torch.Tensor]
) -> List[str]:
    """Copy LoRA parameters into ``model`` in-place.

    Args:
        model: The target model (should already have LoRA modules instantiated).
        lora_state: Mapping from parameter name to tensor, typically the subset
            extracted by :func:`split_lora_state_dict` where keys contain ``lora_``.

    Returns:
        A list of parameter names that were present in ``lora_state`` but missing
        in the model (useful for logging warnings).
    """
...
def load_state_dict_into_model(
    ...
    base_state, lora_state = split_lora_state_dict(state_dict)
    ...
    if lora_state:
        pending = getattr(target_model, "_pending_lora_state_dict", {})
        pending.update(lora_state)
        setattr(target_model, "_pending_lora_state_dict", pending)
