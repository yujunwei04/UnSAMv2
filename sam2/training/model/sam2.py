...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(image_encoder, memory_attention, memory_encoder, temperature=temperature, fourier_dim=fourier_dim, lora_rank=lora_rank, **kwargs)
        ...
        if freeze_image_encoder:
            ...
            for n, p in self.named_parameters():
                ...
                if 'maskmem_tpos_enc' in n:
                    p.requires_grad = False
                if 'no_mem_embed' in n:
                    p.requires_grad = False
                if 'no_mem_pos_enc' in n:
                    p.requires_grad = False
                if 'no_obj_embed_spatial' in n:
                    p.requires_grad = False
    ...
    def forward(self, input: BatchedVideoDatapoint):
        ...
        # backbone_out["granularities_per_frame"]
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        granularities_per_frame = {
            stage_id: granularities.unsqueeze(1)  # [B, 1]
            for stage_id, granularities in enumerate(input.granularities)
        }
        backbone_out["gt_masks_per_frame"] = gt_masks_per_frame
        backbone_out["granularities_per_frame"] = granularities_per_frame
    ...
    def forward_tracking(
        ...
        _, num_images, _, _, _ = input.img_batch.shape
        for stage_id in processing_order:
            ...
            current_out = self.track_step(
                ...
                granularities=backbone_out["granularities_per_frame"],
    ...
    def track_step(
        ...
        if granularities is None:
            granularities = {0: torch.ones_like(point_inputs["point_labels"])}
        ...
        current_out, sam_outputs, high_res_features, pix_feat = self._track_step(
            ...
            gra=granularities,
            granularity_embeddings=self.embed_granularity(granularities),
        )
        ...
        num_iters = self.num_correction_pt_per_frame

        # Optionally, sample correction points iteratively to correct the mask
        if frame_idx in frames_to_add_correction_pt and num_iters > 0:
    ...
    def _iter_correct_pt_sampling(
        ...
        # follow graco's correction point sampling method
        # num_iters = random.randint(0, self.num_correction_pt_per_frame)
