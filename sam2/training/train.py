...
def add_pythonpath_to_sys_path():
    sam2_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    graco_path = os.path.join(sam2_root, 'GraCo')
    ...
    if graco_path not in sys.path:
        sys.path.insert(0, graco_path)
        print(f"Added {graco_path} to sys.path")
        
    if "PYTHONPATH" not in os.environ or not os.environ["PYTHONPATH"]:
        return
    sys.path = os.environ["PYTHONPATH"].split(":") + sys.path
    print(f"Added PYTHONPATH to sys.path: {sys.path}")
