...
class Trainer:
    ...
    def _call_model_initializer(self):
        ...
        if model_weight_initializer is not None:
            ...
            self.model = model_weight_initializer(model=self.model, strict=True)
            
            self._setup_lora_after_loading()
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        if hasattr(self, "optim") and self.optim is not None:
            ...
            self._pending_optimizer_state_dict = checkpoint["optimizer"]
    ...
    def _setup_lora_after_loading(self):
        ...
        self._load_pending_lora_weights(model)

        self._verify_lora_setup()
        self._log_lora_weight_samples()
        ...
        for name, param in self.model.named_parameters():
            if 'lora_' in name:
                if param.requires_grad:
                    ...
                    logging.warning(f"LoRA parameter {name} is frozen!")
            elif 'original_layer' in name:
                if param.requires_grad:
                    logging.warning(f"Original layer parameter {name} is not frozen!")
                else:
                    frozen_params_count += 1

        logging.info(f"LoRA setup verification: {lora_params_count} LoRA parameters trainable, "
                    f"{frozen_params_count} original parameters frozen")

    def _load_pending_lora_weights(self, model):
        ...
        logging.info("Loaded %d LoRA tensors from checkpoint.", len(pending))
        delattr(model, "_pending_lora_state_dict")

    def _log_lora_weight_samples(self, limit: int = 5):
        """Log stats for a few LoRA tensors to help verify correct loading."""
        logged = 0
        for name, param in self.model.named_parameters():
            ...
            logging.info(
                "LoRA weight sample %s | mean %.6f | std %.6f | max_abs %.6f",
                name,
                mean,
                std,
                max_abs,
            ...
            if logged >= limit:
                break
        if logged == 0:
            logging.warning("LoRA weight sample logging found no tensors.")
    ...
    def _step(
        ...
        # In _step method, before model(batch)
        has_nan = False
        for name, tensor in batch.__dict__.items():
            if isinstance(tensor, torch.Tensor) and torch.isnan(tensor).any():
                logging.warning(f"NaN found in batch.{name} before model forward")
                has_nan = True
                break
        if has_nan:
            # Skip this batch
            dummy_loss = torch.tensor(0.0, device=self.device, requires_grad=True)
            return {"Losses/{phase}_{key}_loss": dummy_loss}, 0, {}
        outputs = model(batch)
        targets = batch.masks
        batch_size = len(batch.img_batch)

        key = batch.dict_key  # key for dataset
        # Extract threshold values from model outputs if available
        threshold_values = None
        if outputs and len(outputs) > 0 and "threshold_values" in outputs[0]:
            threshold_values = outputs[0]["threshold_values"]
        loss = self.loss[key](outputs, targets, batch.granularities, threshold_values)
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            for name, tensor in batch.__dict__.items():
                if isinstance(tensor, torch.Tensor) and torch.isnan(tensor).any():
                    logging.warning(f"NaN found in batch.{name} in batch {data_iter}, skipping")
                    has_nan = True
                    break
            ...
            try:
                ...
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    ...
                    for j, param_group in enumerate(self.optim.optimizer.param_groups):
                        for option in self.optim.schedulers[j]:
                            ...
                            log_key = os.path.join("Optim", f"{optim_prefix}", option)
                            scheduler_logs[log_key] = param_group[option]
                            self.logger.log(
                                log_key,
                                param_group[option],
                                self.steps[phase],
                ...
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    ...
                    step_stats = {}
                    for progress_meter in progress.meters:
                        log_key = os.path.join("Step_Stats", phase, progress_meter.name)
                        step_stats[log_key] = progress_meter.val
                        self.logger.log(
                            log_key,
                            progress_meter.val,
                            self.steps[phase],
            ...
            for meter_subkey, meter_value in meter_output.items():
                ...
                if tracked_meter_key not in self.best_meter_values or is_better_check(
                    ...
                    if (
                        ...
                        ckpt_name = f"{tracked_meter_key.replace('/', '_')}_epoch_{int(self.epoch)+1}"
                        checkpoint_save_keys.append(ckpt_name)
                        try:
                            metric_val_jsonable = float(meter_value)
                        except Exception:
                            try:
                                metric_val_jsonable = meter_value.item() if hasattr(meter_value, "item") else meter_value
                                try:
                                    metric_val_jsonable = float(metric_val_jsonable)
                                except Exception:
                                    pass
                            except Exception:
                                metric_val_jsonable = str(meter_value)
                        best_updates.append({
                            "metric": tracked_meter_key,
                            "checkpoint": ckpt_name,
                            "epoch": int(self.epoch) + 1,
                            "value": metric_val_jsonable,
                        })

        if len(best_updates) > 0 and self.distributed_rank == 0:
            try:
                log_path = os.path.join(self.logging_conf.log_dir, "best_ckpt_updates.jsonl")
                with g_pathmgr.open(log_path, "a") as f:
                    f.write(json.dumps({
                        "epoch": int(self.epoch) + 1,
                        "phases": phases,
                        "updates": best_updates,
                    }) + "\n")
                logging.info(f"Best checkpoints updated at epoch {int(self.epoch)+1}: {[u['checkpoint'] for u in best_updates]}")
            except Exception as e:
                logging.warning(f"Failed to write best checkpoint updates: {e}")
    ...
    def _construct_optimizers(self):
        ...
        if hasattr(self, "_pending_optimizer_state_dict"):
            try:
                self.optim.optimizer.load_state_dict(self._pending_optimizer_state_dict)
            finally:
                delattr(self, "_pending_optimizer_state_dict")
...
def print_model_summary(model: torch.nn.Module, log_dir: str = ""):
    ...
    for name, param in model.named_parameters(**param_kwargs):
        if param.requires_grad:
            logging.info(f"Parameter {name} is trainable")
            # number of parameters in current layer
            param_count = param.numel()
            logging.info(f"Parameter {name} has {param_count} parameters ({get_human_readable_count(param_count)})")
